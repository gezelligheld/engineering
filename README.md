...
- 其他适配问题： ios 底部前进后退区域挤压高度的问题、覆盖默认长按问题、下载问题、自动播放媒体问题等

##### 微信小程序

- 双线程架构

  - 渲染层：使用 WebView 进行渲染，为贴近原生应用体验，多个页面时会存在多个 WebView 线程。wxml、wxss、wxs 都工作在渲染层
  - 逻辑层：运行 js 脚本
  - native 层：负责处理渲染层和逻辑层的通信、网络请求、离线存储等，渲染层和逻辑层的通信实际上是线程间的通信，会存在一定的延迟

- 运行环境：小程序在微信开发者工具、IOS、Android 等环境下脚本执行环境、渲染引擎都有所不同，具体表现有以下差异

  - js 语法和 api 支持不一致，通过编译成 es5 和内置的 polyfill 规避
  - WXSS 渲染表现不一致，需要真机调试

- 启动流程（App.onLaunch -> App.onShow -> Page.onLoad -> Page.onShow -> Page.onReady）

  1. 资源准备：准备小程序版本、权限等相关信息，准备小程序进程、系统组件、WebView 等运行环境，并下载代码包。当代码包更新时，只需要下载算法生成的增量包即可
  2. 代码注入：逻辑层代码注入，触发 App.onLaunch 和 App.onShow，同时渲染层代码注入
  3. 渲染：逻辑层生成初始数据发送给渲染层，并依次触发 Page.onLoad、Page.onShow。结合渲染层信息和初始数据进行渲染，最后触发 Page.onReady

- 页面切换流程（当前页面的 onHide/onUnload -> 目标页面的 onLoad -> 目标页面的 onShow）

  1. 分包加载：触发页面切换后，如果目标页面处于分包中，会先加载分包
  2. 初始化：渲染层会创建 WebView，并注入小程序基础库、主包、分包代码，逻辑层触发上一个页面的 onHide/onUnload，更新路由栈并发送数据给渲染层，触发目标页面的 onLoad、onShow。为了降低切换页面耗时，页面渲染完成后会进行预加载供页面切换使用
  3. 渲染：结合渲染层信息和初始数据进行渲染，触发 onReady，然后进行页面切换动画

- 状态更新流程

  1. 逻辑层虚拟 dom 树更新，触发生命周期、observer 等，然后将数据发送给渲染层
  2. 渲染层虚拟 dom 树更新，与上一次的 dom 树对比，进行节点的增删改操作，最终触发真实 dom 变化

- 特有语法

  - WXML：语法类似 vue。内置的编译器会将 WXML 编译成 js 函数，获取到页面信息后会输出一段描述页面信息的 json，用于生成 html
  - WXSS：在 CSS 的基础上做了扩充，新增可以自适应的尺寸单位 rpx，支持导入样式。内置的编译器会解析样式文件间的引用关系、将 rpx 转为 px，用于生成 css
  - WXS：小程序脚本语言，语法类似 es5，可以不通过逻辑层直接作用于渲染层

- 组件

  - 生命周期：created（组件实例创建）-> attached（组件初始化完成）-> ready（组件布局完成）--> detached（组件从节点树移除）
  - 逻辑抽离和复用：behaviors，类似混入，可以将 behaviors 中的状态和方法合并到调用方，还支持组件扩展
  - 事件通信：triggerEvent
  - 组件模型：与 WebComponents 标准中的 ShadowDOM 类似，默认有自己的样式作用域
  - 原生组件：类似 map 这样的原生组件 WebView 只渲染一个占位元素，其余交由客户端原生处理，性能更好

- 性能优化

  - 启动时
    - 分包加载以减小主包体积，支持独立分包、分包预加载
    - 移除未使用的组件和插件
    - 控制代码包内资源文件的大小
    - 按需引入和用时注入
    - 减少阻塞启动过程中同步调用的生命周期
    - 初始渲染缓存，提前将初始数据给到渲染层渲染，无需等待逻辑层初始化完成
    - 提前进行首屏数据获取
    - 骨架屏
    - 合理规划版本发布，版本发布后会增加启动耗时
  - 运行时
    - 合理使用 setData，动画和 scroll 事件尽量绕开逻辑层，尽量减少渲染层和逻辑层两个线程间通信的次数和数据量
    - 页面切换时，避免在 onHide/onUnload 执行耗时操作
    - 内存优化，及时释放小程序实例、销毁定时器、解绑事件等

...
- 双线程架构
  ...
  - 渲染层：使用 WebView 进行渲染，为贴近原生应用体验，多个页面时会存在多个WebView线程。wxml、wxss、wxs都工作在渲染层
  - 逻辑层：运行js脚本
  - native层：负责处理渲染层和逻辑层的通信、网络请求、离线存储等，渲染层和逻辑层的通信实际上是线程间的通信，会存在一定的延迟

- 运行环境：小程序在微信开发者工具、IOS、Android等环境下脚本执行环境、渲染引擎都有所不同，具体表现有以下差异

  - js语法和api支持不一致，通过编译成es5和内置的polyfill规避
  - WXSS 渲染表现不一致，需要真机调试

- 启动流程（App.onLaunch -> App.onShow -> Page.onLoad -> Page.onShow -> Page.onReady）

  1. 资源准备：准备小程序版本、权限等相关信息，准备小程序进程、系统组件、WebView等运行环境，并下载代码包。当代码包更新时，只需要下载算法生成的增量包即可
  2. 代码注入：逻辑层代码注入，触发App.onLaunch 和 App.onShow，同时渲染层代码注入
...
- 页面切换流程（当前页面的 onHide/onUnload -> 目标页面的 onLoad -> 目标页面的 onShow）
  ...
  2. 初始化：渲染层会创建WebView，并注入小程序基础库、主包、分包代码，逻辑层触发上一个页面的onHide/onUnload，更新路由栈并发送数据给渲染层，触发目标页面的onLoad、onShow。为了降低切换页面耗时，页面渲染完成后会进行预加载供页面切换使用
  3. 渲染：结合渲染层信息和初始数据进行渲染，触发onReady，然后进行页面切换动画

- 状态更新流程

  1. 逻辑层虚拟dom树更新，触发生命周期、observer等，然后将数据发送给渲染层
  2. 渲染层虚拟dom树更新，与上一次的dom树对比，进行节点的增删改操作，最终触发真实dom变化

- 特有语法

  - WXML：语法类似vue。内置的编译器会将WXML编译成js函数，获取到页面信息后会输出一段描述页面信息的json，用于生成html
...
- 性能优化
  ...
  - 运行时
    - 合理使用setData，动画和scroll事件尽量绕开逻辑层，尽量减少渲染层和逻辑层两个线程间通信的次数和数据量
    - 页面切换时，避免在onHide/onUnload执行耗时操作

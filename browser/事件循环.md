...
4. poll 阶段：进入 poll 阶段进行轮询，首先会检查是否存在对应 I/O 的 callback，如果存在依次插入轮询队列并执行，并清空随之产生的 Process.nextTick 以及 micro 任务，直到队列清空。之后如果有 setImmediate 回调任务，将结束 poll 阶段进入 check 阶段，否则 event loop 阻塞在 poll 阶段等待回调被添加到队列中，跳过 check 和 close callbacks 阶段，然后进入下一周期的 event loop
